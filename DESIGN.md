...
- cluster map: `--cluster-f0`…`--cluster-f6` (one colour per SONARA facet, in facet order), `--cluster-kept` and `--cluster-differs` (cells where fewer or more of the library than a random track sits closer to the references), `--cluster-candidate`, `--cluster-selected` and `--cluster-reference` (map points and chart lines), `--cluster-library` and `--cluster-library-iqr` (library p5–p95 and p25–p75 bands), `--font-mono` (system monospace, for the map's small numeric labels only);
...
- cluster map dialog (`ClusterMapDialog.tsx`, `ClusterMapPlot.tsx`): the REFERENCE map of the current search, held in memory only until the model, layer, references or limit change; `role="dialog"` with `aria-modal`, closed by Escape. An SVG polar map: radius is the SONARA distance to the references (or one facet of it), angle the facets that make it, library-percentile rings, stars for references; below it a candidates × facets percentile table. Rail tabs: candidate evidence, output summary, method. Its copy is Russian.
...
State changes use short transitions (120–240ms) on background, border, color, or transform, and busy indicators spin. There is no other decorative animation, with one existing exception: the player dock's spectrum edge travels on a 14-second loop (`player-edge-travel`) and holds still under `prefers-reduced-motion: reduce`. Spinners and transitions are not gated by `prefers-reduced-motion`. The cluster map dialog is the exception: it opens with opacity and a 0.97→1 scale over 220ms and closes over 160ms (`cubic-bezier(0.23, 1, 0.32, 1)`), its backdrop fading with it, and under `prefers-reduced-motion: reduce` it only fades. Every button that does not submit has `type="button"`, a title or label, and a clear disabled state. Preserve keyboard access through native controls and existing `ResultRow` behavior.
