...
Audio Doctor `--apply` может переписывать только файлы, которые инструмент определил как `REPAIRABLE`; dry-run не пишет аудио. UI/API repair mode требует `APPLY REPAIR`. Audio Dedup apply/delete может удалить подтверждённые дубли только после точного подтверждения.
