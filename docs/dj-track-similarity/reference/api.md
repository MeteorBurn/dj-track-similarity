...
Audience: developers and frontend integrators  
Goal: show current endpoint groups and the main payload models  
Type: reference

The FastAPI app is created by `dj_track_similarity.api:create_app`. It mounts
the built docs at `/docs` when `docs/dj-track-similarity/site` exists and the
frontend bundle at `/` when `frontend/dist` exists.
