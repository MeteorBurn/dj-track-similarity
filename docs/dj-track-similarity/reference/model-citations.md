...
| --- | --- | --- | --- |
| SONARA | `sonara==0.3.1`, schema 5, playlist mode with BPM range `70.0..180.0`; Core uses bundled vocalness v2 and each output has its own signature | [kkollsga/sonara](https://github.com/kkollsga/sonara), [v0.3.1 release](https://github.com/kkollsga/sonara/releases/tag/v0.3.1) | Upstream is MIT. Preserve its license and check terms before redistribution. |
...
`pyproject.toml` and `uv.lock` pin `sonara==0.3.1`. PyPI provides `cp310-abi3` wheels for supported
platforms. The runtime contract requires that exact package and schema version.
