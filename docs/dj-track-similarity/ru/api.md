...
их отсутствии — SONARA fallback. Ordered preview также соблюдает строгий artist
guard: один известный исполнитель может появиться в одном preview максимум один
раз. Manual seeds входят в ответ как `seed_anchor`, но повтор известного
исполнителя среди manual seeds отклоняется. Auto anchors и остальные позиции
семплируются из mode-scored pools, поэтому повторные вызовы без `random_seed`
могут возвращать разные, но связанные сеты.
