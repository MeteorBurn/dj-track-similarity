...
def test_ablation_uses_current_typed_records_and_manual_labels() -> None:
    ...
    metrics = report["variants"]["source:mert_v2"]["metrics"]
...
def test_ablation_rrf_and_classifier_removal_preserve_ranking_invariants() -> None:
        ...
        events=(
            {
                ...
                "sources": {"mert_v2": {"rank": 2}, "maest": {"rank": 2}},
            },
            {"candidate_track_id": 3, "sources": {"mert_v2": {"rank": 1}}},
            {
                "candidate_track_id": 4,
                "sources": {"mert_v2": {"rank": 3}},
                "score_breakdown": {
                    "classifier_support": {
    ...
    assert report["counts"]["signals_seen"] == ["mert_v2", "maest", "classifiers"]
...
def test_ablation_weighted_rrf_and_judged_gate_do_not_rewrite_rank_positions() -> None:
    ...
    repository.add_session(
        events=(
            {
                "candidate_track_id": 2,
                "sources": {"mert_v2": {"rank": 1}, "maest": {"rank": 10}},
            },
            {
                "candidate_track_id": 3,
                "sources": {"mert_v2": {"rank": 10}, "maest": {"rank": 1}},
            },
        )
    )
    ...
    report = build_source_ablation_report(
        ...
        k_values=(1,),
        rrf_k=60,
        score_profile=profile({"mert_v2": 0.1, "maest": 0.9}),
        judged_only=True,
    )
