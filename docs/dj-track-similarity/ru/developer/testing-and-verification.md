...
> Для кого: Для разработчиков, которые выбирают проверки после изменения.
...
сайт. После свежего checkout или изменения пакетов `.vale.ini` один раз выполните
`npm run vale:sync`. Для того же отчёта без завершения с ошибкой используйте `npm run lint:style`.

## Сфокусированные примеры
