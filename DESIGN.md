...
- cluster map: `--cluster-1` to `--cluster-6` (six categorical map colors in plain hex, which Plotly reads), `--cluster-glow-opacity` (the dark theme's neon glow, 0 in the light theme), `--font-mono` (system monospace, for the map's small numeric micro-labels only);
...
- cluster map dialog (`ClusterMapDialog.tsx`, `ClusterMapPlot.tsx`): the REFERENCE orbit map, kept mounted while its build is cached so it reopens with the same zoom; `role="dialog"` with `aria-modal`, closed by Escape; legend chips isolate one cluster (`aria-pressed`); rail tabs switch between Clusters, Anomalies, and Layer (Model for a model without layers).
