...
- cluster map dialog (`ClusterMapDialog.tsx`, `ClusterMapPlot.tsx`): the REFERENCE orbit map, kept mounted while its build is cached so it reopens with the same zoom; `role="dialog"` with `aria-modal`, closed by Escape; the layer's exceptions wear their own colour and a ring over the swarm; rail tabs switch between Tracks and Layer (Model for a model without layers).
