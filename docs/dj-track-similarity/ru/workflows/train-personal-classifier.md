...
```text
ваши метки -> обученный кандидат -> опубликованный профиль -> оценки треков -> CLASS / SET / Hybrid
```

Итоговая оценка не записывается в аудиотег и не является объективной истиной. Это повторно
используемый сигнал ранжирования одного профиля. По нему можно фильтровать библиотеку и направлять
SET к понятию или от него. Опубликованный манифест также может открыть эту оценку как
диагностический сигнал Hybrid.

Rhythm Lab — рабочее пространство классификаторов. Оно использует основную библиотеку SQLite как
...
Публикация копирует выбранный рабочий артефакт в
...
После ревизии признаков SONARA зависимые оценки основной библиотеки и предсказания Rhythm Lab
...
остаётся заблокированным: его сигнатура манифеста не может оценивать актуальные треки.
