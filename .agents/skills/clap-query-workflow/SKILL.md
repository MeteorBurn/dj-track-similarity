...
python .agents\skills\clap-query-workflow\scripts\project_text_search.py `
  --model mulan `
  --query "broken drums" `
...
median library rank. Those labels belong to the catalog of `database\volumes_old.sqlite`; the
currently selected library has a different catalog and does not join to them.

## Bundled Resources

- `scripts/project_text_search.py`: posts a prompt bank to the running API for either text model.
- `scripts/validate_prompt_bank.py`: model-free structural check for a bank you are drafting, before
  it becomes a preset.
- `scripts/score_prompt_bank.py`: standalone audio-file scorer for experiments outside the project
  DB. It must load PyTorch checkpoints with `weights_only=True`.
- `references/clap_prompting_reference.md`: LAION-CLAP prompt engineering background.
...
- API helper smoke: `python .agents\skills\clap-query-workflow\scripts\project_text_search.py --help`
