...
test("topbar log and process controls are separate actions", () => {
  ...
  assert.match(actionsBlock, /log-frame-button[\s\S]*rhythm-lab-launch-button[\s\S]*rhythm-lab-stop-button[\s\S]*stop-active-stage-button[\s\S]*process-indicator[\s\S]*notice/);
