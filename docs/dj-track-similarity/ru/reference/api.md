...
Ниже описан текущий интерфейс backend. Клиент React использует соответствующие формы треков, поля
запросов и типы ответов; эта страница остаётся справочником диапазонов и правил проверки конечных
точек.
...
Для нового пути создаются Core и обязательная база Artifacts. Evaluation остаётся необязательной.
Неполный или структурно несовместимый комплект отклоняется и не мигрирует при обычном запуске.
...
проверки плана в режиме только чтения.
