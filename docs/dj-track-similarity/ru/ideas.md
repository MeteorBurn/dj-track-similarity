...
- BPM/key из file tags, а при их отсутствии SONARA fallback, как мягкий сигнал
  порядка переходов, плюс опциональные явные BPM-траектории для slow-to-fast
  или fast-to-slow сетов.
...
- `Diversity`: `0.00-1.00`; ниже = ближе к anchors, выше = шире подборка при
  сохранении режима.
- `BPM mode`: `General BPM - transition`, `Low to high - climb` или
  `High to low - descend`.
- `BPM change`: slow, medium или fast движение темпа по preview.
- `Start BPM` и `Target BPM`: необязательные явные границы темпа; пустые
  значения выводятся из первого seed/anchor и BPM-диапазона библиотеки.
...
- `bpm_mode`: `general`, `low_to_high` или `high_to_low`.
- `bpm_change`: `slow`, `medium` или `fast`.
- `bpm_start`: необязательный явный стартовый BPM.
- `bpm_target`: необязательный явный целевой BPM.
