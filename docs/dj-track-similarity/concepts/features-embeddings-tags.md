...
Complete beat positions, onset positions, chord labels/events, tempo curves, energy curves, structure segments, loudness curves, and downbeat arrays live in the adjacent `*.timeline.sqlite` database. The optional SONARA embedding and fingerprint live in `*.representations.sqlite`. The metadata dialog does not load those values: its track row contains only field-name manifests, so it can show that Timeline or Representations data exists without expanding the main Core database. Time signature, time-signature confidence, tempo variability, and compact curve summaries stay in Core metadata. The actively searched MAEST, MERT, MuQ, and CLAP vectors remain in one indexed `embeddings` table in Core.
