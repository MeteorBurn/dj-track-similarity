"""Acceptance tests for the current Core and sidecar DDL.

Run with:
    python -m pytest tests/test_db_ddl.py --override-ini addopts= -q
...
def _open_database() -> sqlite3.Connection:
    """Return an in-memory connection with the current Core DDL applied."""
...
def test_new_database_matches_current_core_structure() -> None:
    """Check the current Core structure and constraints."""
    conn = _open_database()
    conn.execute(
        "INSERT INTO library_catalog(singleton_id, catalog_uuid, created_at, updated_at) "
        "VALUES (1, 'test-catalog', 'created', 'updated')"
    )
    report = inspect_database_structure(conn, "core")
    ...
    # --- classifier_scores has the current classification fields ---
...
def test_create_core_schema_from_path_string(tmp_path: pytest.TempPathFactory) -> None:
    """create_core_schema() accepts a path string and creates the schema."""
    db_path = str(tmp_path / "test.sqlite")
    create_core_schema(db_path)
    conn = sqlite3.connect(db_path)
    conn.row_factory = sqlite3.Row
    assert "file_modified_ns" in _columns(conn, "tracks")
    conn.close()


def test_create_core_schema_memory_string() -> None:
    """create_core_schema(':memory:') exits cleanly with no output."""
    # Should not raise
    create_core_schema(":memory:")
...
def test_classifier_scores_score_bucket_constraint() -> None:
    ...
    with pytest.raises(sqlite3.IntegrityError):
        conn.execute(
            ...
            ") VALUES (1,'uuid-1','k',1,'features','[]','pos','pos','invalid',0.5,0.5,'{}','2026-01-01T00:00:00.000000Z')"
        )
    conn.close()
...
def test_dataclasses_are_importable() -> None:
    ...
    conn = _open_database()
    assert tuple(
        field.name for field in dataclasses.fields(SonaraRow)
    ) == _column_names(conn, "sonara")
    assert tuple(
        field.name for field in dataclasses.fields(ClassifierScoreRecord)
    ) == _column_names(conn, "classifier_scores")
    conn.close()
...
def test_artifacts_sidecar_schema() -> None:
    """Full structural check for the artifacts sidecar schema."""
    conn = _open_artifacts(catalog_uuid="test-uuid")
    report = inspect_database_structure(conn, "artifacts")
    assert report.is_current, report
    ...
    base_emb = (
        ...
        "(track_id, track_uuid, content_generation, dim, normalization, embedding_blob, analyzed_at)"
        " VALUES (1, 'uuid-1', 1, ?, 'l2', ?, '2026-01-01T00:00:00.000000Z')"
    )
    ...
    with pytest.raises(sqlite3.IntegrityError):
        ...
        conn.execute(
            ...
            "(track_id, track_uuid, content_generation, dim, normalization, embedding_blob, analyzed_at)"
            " VALUES (1, 'uuid-1', 1, ?, 'wrong', ?, '2026-01-01T00:00:00.000000Z')",
            (dim, good_blob),
        )
    conn.rollback()
...
def test_artifacts_sonara_fingerprint_blob_length_check() -> None:
    ...
    base_fp = (
        ...
        " VALUES (1, 'uuid-1', 1, 'v1', ?, 'little', ?, '2026-01-01T00:00:00.000000Z')"
    )
    # Correct length — must succeed
...
def test_artifacts_sonara_timeline_payload_json_check() -> None:
    ...
    base_tl = (
        ...
        "(track_id, track_uuid, content_generation, payload_json, analyzed_at)"
        " VALUES (1, 'uuid-1', 1, ?, '2026-01-01T00:00:00.000000Z')"
    )
...
def test_artifacts_storage_metadata_is_immutable_even_for_insert_or_replace() -> None:
    ...
    original = tuple(
        conn.execute(
            """
            SELECT singleton_id, catalog_uuid, created_at, updated_at
