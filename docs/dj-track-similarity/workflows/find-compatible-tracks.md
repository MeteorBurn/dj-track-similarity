...
1. Search the library for the reference track in panel `2. Библиотека и прослушивание` (library and
   listening).
2. Press the magnifier icon on its row, titled `Seed`. The track appears as a chip above the tab
   strip in panel `3. Поиск и прослушивание` (search and listening).
...
7. Add good candidates with the plus icon titled `В сет` (into the set), then export or save them as
   a Rhythm Lab collection.
...
buttons are inside the collapsible `Сет и экспорт` (set and export) block.
