...
- cluster map: `--cluster-agree`, `--cluster-neutral` and `--cluster-disagree` (the diverging SONARA-closeness scale from the closest tracks through a random one to the farthest, in plain hex, which Plotly reads), `--cluster-glow-opacity` (the dark theme's neon glow, 0 in the light theme), `--font-mono` (system monospace, for the map's small numeric micro-labels only);
...
- cluster map dialog (`ClusterMapDialog.tsx`, `ClusterMapPlot.tsx`): the REFERENCE orbit map, kept mounted while its build is cached so it reopens with the same zoom; `role="dialog"` with `aria-modal`, closed by Escape; points wear their SONARA closeness to the references on the diverging scale; rail tabs switch between Tracks and Layer (Model for a model without layers).
