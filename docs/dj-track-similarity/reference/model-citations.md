...
| --- | --- | --- | --- |
| SONARA | Current tested package `0.3.5`, playlist mode with the per-library BPM range chosen at import; Core uses the bundled vocalness selector | [kkollsga/sonara](https://github.com/kkollsga/sonara), [v0.3.5 release](https://github.com/kkollsga/sonara/releases/tag/v0.3.5) | Upstream is MIT. Preserve its license and check terms before redistribution. |
...
| MuQ-MuLan | `OpenMuQ/MuQ-MuLan-large` through `MuQMuLan.from_pretrained(...)` in the `muq` package | [tencent-ailab/muq](https://github.com/tencent-ailab/muq), [MuQ-MuLan-large checkpoint](https://huggingface.co/OpenMuQ/MuQ-MuLan-large) | Upstream code is MIT. The model card marks the weights CC-BY-NC-4.0. |
...
`pyproject.toml` and `uv.lock` record `sonara==0.3.5` for the current tested environment. PyPI
provides `cp310-abi3` wheels for supported platforms. A future update may change the version after
the affected project paths and dependencies are adapted and checked.
