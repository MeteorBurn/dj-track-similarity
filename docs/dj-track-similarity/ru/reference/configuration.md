...
| Предлагаемый путь интерактивного скрипта Windows | `database\volumes.sqlite` в корне репозитория |
...
Rhythm Lab использует `rhythm_lab.sqlite` как единственную записываемую базу состояния. При запуске
из основного приложения треки и анализ читаются из выбранной пары Core/Artifacts, а профили, метки,
предсказания, очереди и контрольные точки остаются в этой базе Lab.
...
При запуске без аргументов скрипт предлагает путь `database\volumes.sqlite` в корне репозитория, а
затем запрашивает локальный режим или доступ из локальной сети. Подтверждённый путь передаётся только
после завершения обоих запросов.

Для запуска без запросов выполните `run_server.cmd local --db .\database\volumes.sqlite` или
замените `local` на `lan`. Явные команды режима используют только переданные аргументы. Прямая
команда `dj-sim serve` без `--db` по-прежнему не создаёт базу и ожидает выбора через API или диалог.
