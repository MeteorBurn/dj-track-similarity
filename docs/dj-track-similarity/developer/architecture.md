...
- `audio_loader.py`: one lazy TorchCodec `0.16` decode shared by the generic ML families calls
  ...
  fails, the model runner uses tolerant full-track FFmpeg decoding to produce mono `float32` PCM;
  the adapter still owns resampling and window selection. `WavDecoder` is not used because it
