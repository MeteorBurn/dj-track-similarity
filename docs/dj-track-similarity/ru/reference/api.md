...
> Для кого: Для пользователей и сопровождающих, которые автоматизируют локальное приложение FastAPI.
> Задача: Сводно описать семейства эндпоинтов и ключевые правила данных.
> Тип: Справочник
...
| `GET` | `/api/library/summary` | количества треков, анализа, отметок и классификаторов |
...
Эндпоинт Timeline возвращает полные сохранённые `beats`, `onset_frames`, `chord_sequence`,
...
| `POST` | `/api/classifiers/analyze` | расчёт выбранных классификаторов; пустой список означает все совместимые |
| `POST` | `/api/classifiers/{classifier_key}/analyze` | расчёт одного классификатора |
...
Совокупное тело классификатора: `{ "classifier_keys": [], "limit": null }`. Готовность зависит от
манифеста, общее количество считает готовые пары классификатор–трек, а неготовые исключаются вместо
ошибки. Конвейер выбирает `sonara`, `ml` и/или `classifiers`, общий лимит и вложенные настройки.
...
полям-манифестам трека; точную проверку выбранных результатов выполняет планировщик анализа.
...
| Метод | Путь | Назначение |
| --- | --- | --- |
| `POST` | `/api/search` | поиск MERT по опорным трекам |
| `POST` | `/api/search/sonara` | поиск SONARA по опорным трекам |
| `POST` | `/api/search/text` | текстовый поиск CLAP |
| `POST` | `/api/search/hybrid` | взвешенный просмотр Hybrid |
| `POST` | `/api/set-builder/generate` | черновик Smart Set Builder |
| `POST` | `/api/reference/compare` | группы Reference Compare для одного ориентира |
| `POST` | `/api/reference/compare/verdict` | сохранение одного вердикта прослушивания |

Важные диапазоны:

- списки опорных треков для Hybrid и его обратной связи — `1..5` уникальных ID;
...
| `GET` | `/api/audio-doctor/jobs/{job_id}/report/xlsx` | загрузка XLSX |
...
| `GET` | `/api/audio-dedup/jobs/{job_id}/report/xlsx` | загрузка XLSX |
...
Остановка требует заголовок `X-DJ-Track-Similarity-Action: shutdown-server`.
