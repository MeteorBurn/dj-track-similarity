...
Основной audio-analysis workflow — это одно задание с выбранными моделями. В UI
выберите SONARA, MAEST, MERT и/или CLAP галочками и запустите один анализ. В том
же блоке есть галочка `CLASSIFIERS`: если она включена, оценка продвинутых
классификаторов запускается после выбранных аудиомоделей и только для профилей,
обнаруженных в `models/classifiers/*/model.json`. Треки без нужных входных
данных для классификатора пропускаются. В CLI используйте
`dj-sim analyze --models sonara,maest,mert,clap`; если `--models` не задан,
выбираются все четыре аудиомодели. Трек является кандидатом, если у него
отсутствует хотя бы одна выбранная модель; уже существующие результаты
выбранных моделей пропускаются.
...
затем ресемплит звук к своей нужной частоте.
