...
| Сравнить несколько взглядов моделей или проверить дубликаты | SONARA, MAEST, MERT, MuQ, CLAP | Кандидатов с полным набором данных для LAB, Audio Dedup и классификаторов |
| Сравнить соседей ещё одной модели в LAB | MuQ | Отдельную колонку MuQ в Reference Compare |
| Повторно использовать собственный размеченный признак | CLASSIFIERS | Сохранённые оценки для фильтров CLASS |
...
| --- | --- | --- |
| SONARA | строки признаков Core | поиск по признакам, оценку темпа с учётом уверенности, определение Camelot, диагностику переходов Evaluation, Audio Dedup и входы классификаторов |
| MAEST | жанры и синкопированность в Core, эмбеддинг в Artifacts | показ жанров, запись жанрового тега, LAB Reference Compare, Audio Dedup и вход классификатора |
| MERT | эмбеддинг в Artifacts | поиск по референсному треку, LAB Reference Compare, Audio Dedup и вход классификатора |
| MuQ | эмбеддинг в Artifacts | поиск по референсному треку, LAB Reference Compare, Audio Dedup и вход классификатора |
| CLAP | аудиоэмбеддинг в Artifacts | поиск по референсному треку и тексту, LAB Reference Compare, Audio Dedup и вход классификатора |
| CLASSIFIERS | строки Core `classifier_scores` | фильтры CLASS |
...
используется в поиске по референсному треку, LAB Reference Compare, Audio Dedup и совместимых
манифестах классификаторов. Audio Dedup отключает MuQ, если в явном списке источников отсутствует
`muq`.
