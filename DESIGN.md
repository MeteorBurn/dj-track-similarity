...
- cluster map: `--cluster-swarm` and `--cluster-exception` (the candidates that fit in with the rest in the layer's space and the ones that do not, in plain hex, which Plotly reads), `--cluster-glow-opacity` (the dark theme's neon glow, 0 in the light theme), `--font-mono` (system monospace, for the map's small numeric micro-labels only);
...
- cluster map dialog (`ClusterMapDialog.tsx`, `ClusterMapPlot.tsx`): the REFERENCE orbit map, kept mounted while its build is cached so it reopens with the same zoom; `role="dialog"` with `aria-modal`, closed by Escape; the layer's exceptions wear their own colour and a ring over the swarm; rail tabs switch between the track lists and the layer summary (the model summary for a model without layers); its copy is Russian.
