...
from dj_track_similarity.analysis_models import MERT_EMBEDDING_DIM
...
def test_report_reader_carries_public_current_identity(
    tmp_path: Path,
) -> None:
    core = _load_audio_dedup_core()
    root = tmp_path / "music"
    ...
    with database.connect_artifacts() as artifacts:
        artifacts.execute(
            ...
            (
                ...
                MERT_EMBEDDING_DIM,
                np.pad(
                    np.array((2.0,), dtype="<f4"),
                    (0, MERT_EMBEDDING_DIM - 1),
                ).tobytes(),
                _SCANNED_AT,
            ),
        )
