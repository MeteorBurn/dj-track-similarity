...
def test_build_manifest_binds_current_identity_without_versioned_contracts(
    tmp_path: Path,
    ...
    assert manifest["embedding_count"] == len(snapshot.targets)
    assert manifest["embedding_dim"] == snapshot.matrix.shape[1]
...
def test_artifact_and_target_identity_tampering_are_rejected(
    ...
    identity_tamper = verify_persistent_index(
        repository,
        "mert",
        analysis_output=repository.output,
        index_dir=index_dir,
    ...
    assert identity_tamper.status == "error"
    assert identity_tamper.reasons == ("manifest_invalid",)
    assert "self-hash" in identity_tamper.message
...
def test_ann_accepts_only_explicit_l2_ml_families_and_hnsw_backend(
    ...
    with pytest.raises(
        ValueError,
        match="No active mert/embedding output",
    ):
...
def _repository(tmp_path: Path) -> FakeAnalysisRepository:
    catalog_uuid = "catalog-ann-current"
