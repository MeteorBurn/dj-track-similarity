...
```text
ваши метки -> обученная модель-кандидат -> опубликованный профиль -> оценки треков -> CLASS
```

Итоговая оценка не записывается в аудиотег и не является объективной истиной. Это повторно
используемый сигнал ранжирования одного профиля. По нему можно фильтровать фонотеку через CLASS.
