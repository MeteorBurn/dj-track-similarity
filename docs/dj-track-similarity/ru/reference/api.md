...
Ниже описан активный контракт backend v7. Клиент React использует соответствующие идентичности v7,
формы треков, поля запросов и типы ответов; эта страница остаётся справочником диапазонов и правил
...
| `GET` | `/api/analysis/sonara/releases/status` | готовность точного релиза загруженной исполняемой части для выбранного каталога |
...
Ответ проверки, которая только читает данные, привязан к `catalog_uuid`. Он возвращает
`ready`, `preparation_required` или `unavailable`, ожидаемый и активный хеши релиза и состояние
`current`, `missing`, `stale` или `unavailable` для каждого точного результата. Число готовых строк
не заменяет состояние релиза: у активного релиза может ещё не быть проанализированных треков.
...
упорядоченный процесс с квитанцией, который можно продолжить после прерывания. Сервер повторно
проверяет `catalog_uuid` при резервировании выбранной базы до создания копии или изменения
состояния релиза. Поэтому после смены базы прежнее подтверждение завершается ответом `409`, а не
подготавливает другой каталог. Типизированная завершённая квитанция содержит `catalog_uuid`,
`release_hash`, четыре `contract_hashes`, пути, хеши и размеры копий Core и Artifacts, а также
`core_rows_deleted`, `artifact_rows_deleted` и `classifier_rows_deleted`. Подготовка не запускает
задачу анализа.
...
SET принимает в `sources` значения `mert`, `maest`, `muq` и `clap`. По умолчанию включены все
четыре, а широкое сходство SONARA остаётся обязательным и не входит в этот список. Исходные веса по
умолчанию: `mert=0.30`, `maest=0.18`, `muq=0.15`, `clap=0.22` и `sonara_broad=0.30`; бэкенд
нормализует включённые значения. Пользовательский объект `weights` должен содержать ровно ключи
включённых источников и `sonara_broad`. Если исключить `muq` из `sources`, восстанавливается прежняя
исходная смесь `0.30/0.18/0.22/0.30`.

Hybrid принимает `mert`, `maest`, `muq`, `sonara` и `clap`. По умолчанию включены все пять с
одинаковым нормализованным весом `0.20`. Если исключить `muq` и не задавать свои веса, каждый из
четырёх оставшихся источников получает `0.25`. Ключи пользовательских весов должны точно совпадать
с выбранными источниками.
...
Запрос Audio Dedup принимает в `sources` значения `mert`, `maest`, `muq`, `clap` и необязательный
объект `weights` с точным набором ключей. Исходные веса по умолчанию: `mert=0.43`, `maest=0.32`,
`muq=0.12`, `clap=0.04`. Доступные включённые значения нормализуются при расчёте. Исключение `muq`
сохраняет прежнюю смесь MERT/MAEST/CLAP. Независимое подтверждение MERT и MAEST остаётся обязательным
для безопасного удаления, поэтому одного сигнала MuQ всегда недостаточно и результат остаётся на
ручной проверке.
