...
def test_library_database_bootstraps_bound_core_and_required_artifacts(
    tmp_path: Path,
) -> None:
...
def test_cached_connection_fully_revalidates_same_catalog_file_replacement(
    ...
    def count_validation(
        ...
        *,
        expected_catalog_uuid: str | None = None,
        database_path: str = "<connected database>",
    ) -> str:
        nonlocal calls
        calls += 1
...
def test_library_database_rejects_non_artifacts_schema(tmp_path: Path) -> None:
    ...
    with pytest.raises(
        (RuntimeError, ValueError),
        match=r"(?i)(artifacts|structure|migration)",
    ):
        LibraryDatabase(core_path)
...
def test_library_database_rejects_old_structure_without_creating_sidecars(
    tmp_path: Path,
) -> None:
    ...
    with pytest.raises(
        (RuntimeError, ValueError),
        match=r"(?i)(structure|migrate-database)",
    ):
        LibraryDatabase(core_path)
...
def test_library_database_creates_evaluation_sidecar_only_when_requested(
    ...
    with closing(created) as connection:
        metadata = connection.execute(
            """
            SELECT singleton_id, catalog_uuid
            FROM storage_metadata
            """
        ).fetchone()
    assert metadata is not None
    assert tuple(metadata) == (1, database.catalog_uuid)
...
def test_concurrent_library_database_opens_publish_one_bound_bundle(
    ...
    def open_bundle() -> tuple[str, str]:
        ...
        return core_catalog_uuid, artifacts_catalog_uuid

    with ThreadPoolExecutor(max_workers=workers) as executor:
        results = list(executor.map(lambda _: open_bundle(), range(workers)))

    assert len(results) == workers
    assert len({core_uuid for core_uuid, _ in results}) == 1
    assert len({artifacts_uuid for _, artifacts_uuid in results}) == 1
    assert all(core_uuid == artifacts_uuid for core_uuid, artifacts_uuid in results)
    assert sorted(path.name for path in tmp_path.glob("*.sqlite")) == [
        "library.artifacts.sqlite",
        "library.sqlite",
    ]
...
def test_bootstrap_recovery_never_deletes_nonmatching_existing_target(
    ...
    with pytest.raises(
        (RuntimeError, ValueError),
        match=r"(?i)(structure|migration|artifacts|core)",
    ):
        LibraryDatabase(core_path)
...
def test_eight_process_concurrent_evaluation_creation_is_idempotent(
    ...
    with closing(reopened) as connection:
        ...
        assert [tuple(row) for row in rows] == [(1, database.catalog_uuid)]
