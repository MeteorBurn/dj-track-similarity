...
The tested environment runs SONARA `0.3.6`, built from the upstream repository at tag `v0.3.6`.
`pyproject.toml` pins that version and resolves it to a patched local wheel, which `uv.lock` records
by path and SHA-256. See [Install](../getting-started/install.md) for what that means on another
machine.
