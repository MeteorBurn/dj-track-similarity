...
from .evaluation.candidates import CandidateSourceContribution
from .models import Track
from .transition_diagnostics import COMPONENT_NAMES, V2_COMPONENT_WEIGHTS, TransitionDiagnostics, compute_transition_diagnostics
...
class TransitionCandidate(Protocol):
    ...
    @property
    def source_contributions(self) -> Mapping[str, CandidateSourceContribution]:
        ...
...
def _transition_seed_scope(candidate: TransitionCandidate, seed_tracks_by_id: Mapping[int, Track]) -> tuple[tuple[int, ...], str]:
...
def _source_diagnostic_seed_track_ids(candidate: TransitionCandidate) -> tuple[int, ...]:
...
def _known_seed_track_ids(seed_track_ids: Iterable[JsonValue], seed_tracks_by_id: Mapping[int, Track]) -> tuple[int, ...]:
...
def _mean_transition_diagnostics(
    ...
    *,
    ...
    warnings: list[JsonValue] = [warning for warning in sorted({warning for diagnostic in diagnostics for warning in diagnostic.warnings})]
    available_components: list[JsonValue] = [name for name in COMPONENT_NAMES if components[name] is not None]
    supporting_seed_ids: list[JsonValue] = [seed_track_id for seed_track_id in supporting_seed_track_ids]
    return {
...
def _weighted_mean_components(values: Mapping[str, JsonValue], weights: Mapping[str, float]) -> float | None:
    weighted_sum = 0.0
    total_weight = 0.0
