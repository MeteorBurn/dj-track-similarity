...
- CLI entry is `dj-sim = dj_track_similarity.cli:app`. Commands: `scan`, `relocate-library`, `analyze`, `analyze-classifiers`, `analyze-classifier`, `analyze-pipeline`, `doctor`, `text-search`, `prepare-sonara-release`, `serve`, plus `eval`, `classifier`, and `index` command groups. There is no v6-to-v7 migration command.
...
- Any SONARA update requires a complete SONARA reanalysis. This includes a package/version/build update and any change to the decoder, execution path, analysis mode, sample rate, BPM range, requested features, bundled model, schema, provenance, signature, or project feature revision. The current pinned contract lives in `sonara_contract.py`: SONARA `0.2.9`, schema `4`, mode `playlist`, sample rate `22050`, BPM range `70..180`, feature revision `6`, decoder `sonara-symphonia`, execution path `analyze_batch`. Its output kinds are `core`, `timeline`, `embedding`, and `fingerprint`. Never preserve compatibility by comparing, adapting, translating, or mixing results from the old and new SONARA contracts.
...
- If changing FastAPI request/response contracts, update backend schemas/routes and tests together. The frontend v7 port is a separate deferred task; until it is explicitly authorized, document the compatibility gap instead of editing `frontend/src/api.ts`.
...
- Model-adapter preflight requires the locked loader stack, including `transformers==5.13.0` and `huggingface-hub==1.22.0`, and must fail closed when installed package identity does not match the active analysis contract.
