"""Current single-library database DDL and typed Python domain models.
...
Tables (emission order matches FK dependency order):
  1.  library               — singleton library metadata and scan roots
...
CREATE INDEX idx_tags_sort ON tags(COALESCE(artist,''), COALESCE(title,''), track_id);
"""
...
def create_library_schema(db: "sqlite3.Connection | str") -> None:
    """Create the current single-library schema in *db*.
...
class SonaraRow:
    """SONARA Core row from the ``sonara_features`` table.
